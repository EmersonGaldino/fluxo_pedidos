...
using order.flow.persistence.configuration.Base;
using order.flow.persistence.configuration.exceptions;

namespace order.flow.persistence.repositories.Base;
...
public class RepositoryBase<T> : IRepositoryBase<T> where T : class
{
        ...
        public static string GetInsertFields()
        {
            var values = string.Join(",", typeof(T).GetProperties()
                .Where(
                    x => x.GetCustomAttribute<ColumnAttribute>() != null)
        ...
        public async Task<T> SaveReturnObjectAsync(T item)
        {
            try
            {
                var parameters = new DynamicParameters();
                var sql = GetInsertSql(item, parameters);
                return await _uow.GetConnection().QueryFirstAsync<T>(sql, parameters, _uow.GetTransaction());
            }
            catch (Exception e) { throw e; }
            finally
            {
